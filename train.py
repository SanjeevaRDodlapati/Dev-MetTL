...
import mettl
from mettl import callbacks as cbk

from mettl import metrics as met
from mettl import model as mod


from mettl.utils import format_table, make_dir, EPS, CPG_NAN, get_dna_wlen, get_replicate_names, get_cpg_wlen, \
    get_output_names, get_nb_sample, OUTPUT_SEP, read, add_output_layers, get_first_conv_layer, save_model, \
    get_objectives, data_reader_from_model, copy_weights, load_model, is_input_layer, is_output_layer
from mettl import utils
...
def get_output_stats(output):
    ...
    stats['nb_obs'] = np.sum(output != CPG_NAN)
...
class App(object):
    ...
    def create_parser(self, name):
        ...
        models = sorted(list(mod.list_models().keys()))
        g.add_argument(
            '--dna_model',
    ...
    def build_cpg_model(self):
        ...
        cpg_wlen = get_cpg_wlen(opts.train_files[0], opts.cpg_wlen)

        if os.path.exists(opts.cpg_model[0]):
            log.info('Loading existing CpG model ...')
            src_cpg_model = load_model(opts.cpg_model, log=log.info)
    ...
    def load_full_model(self):
        ...
        src_full_model = load_model(opts.full_model, log=log.info)
        ...
        if nb_replicate != len(replicate_names):
            ...
            for modelName in modelNames:
                if modelName in mettl.model.list_models():
                    modelSpec['dna'] = modelName
                elif modelName in mettl.model.list_models():
                    modelSpec['cpg'] = modelName
                else:
                    modelSpec['joint'] = modelName
            
            # build dna model
            dna_model_builder = mettl.model.get(modelSpec['dna'])(
                l1_decay=opts.l1_decay,
                l2_decay=opts.l2_decay,
                dropout=opts.dropout)
            ...
            dna_model = dna_model_builder(dna_inputs)
            # build cpg model
            cpg_model_builder = mettl.model.get(modelSpec['cpg'])(
                l1_decay=opts.l1_decay,
                l2_decay=opts.l2_decay,
                dropout=opts.dropout)
            ...
            cpg_model = cpg_model_builder(cpg_inputs)
            # build joint model
            joint_model_builder = mettl.model.get(modelSpec['joint'])(
                    l1_decay=opts.l1_decay,
                    l2_decay=opts.l2_decay,
                    dropout=opts.dropout)
    ...
    def build_model(self):
        ...
        output_names = get_output_names(opts.train_files[0],
        ...
        if opts.full_model and os.path.exists(opts.full_model[0]):
            ...
            if dna_model is not None and cpg_model is not None:
                ...
                joint_model_builder = mettl.model.get(opts.joint_model)(
                    l1_decay=opts.l1_decay,
                    l2_decay=opts.l2_decay,
                    dropout=opts.dropout,
                    batch_norm=opts.batch_norm)
                ...
                stem = load_model(opts.model_files, log=log.info)
                if sorted(output_names) == sorted(stem.output_names):
                    return stem
                log.info('Removing existing output layers ...')
                remove_outputs(stem)

        outputs = add_output_layers(stem.outputs[0], output_names, loss_fn=opts.loss_fn)
    ...
    def set_trainability(self, model):
        ...
        if opts.freeze_filter:
            not_trainable.append(get_first_conv_layer(model.layers).name)
    ...
    def main(self, name, opts):
        ...
        for name in output_names:
            output = read(opts.train_files, 'outputs/%s' % name,
        ...
        model.compile(optimizer=optimizer,
                      loss=get_objectives(output_names, opts.loss_fn),
        ...
        replicate_names = get_replicate_names(
            ...
            regex=opts.replicate_names,
            nb_key=opts.nb_replicate)
        data_reader = data_reader_from_model(
            model, replicate_names=replicate_names, loss_fn=opts.loss_fn)
        nb_train_sample = get_nb_sample(opts.train_files,
        ...
        if opts.val_files:
            nb_val_sample = get_nb_sample(opts.val_files,
                                              opts.nb_val_sample)
